...
| 馬達 | **MG310 直流減速馬達 + 500 線 GMR 編碼器（1:30）** | NT$300 | 12V、**500 PPR AB 相 GMR 磁性編碼器**（4x = 2000 counts/rev 馬達軸）、減速 1:30 → 輸出 **100 RPM**、輸出軸 **60,000 counts/rev** |
| 驅動器 | **BTN7971 高電流 H-bridge 模組** | NT$120 | 連續 ~10A（MG310 堵轉 2-4A 綽綽有餘）、PWM+方向控制、過流/過溫保護 |
...
- **MG310 + 500 線 GMR 編碼器（選定）**：較 JGA25-370（11–13 PPR）解析度提升 **~40 倍**（2000 vs 44 counts/rev 馬達軸），M 法速度量測在實用轉速域直接可用（±1% @ 3000 RPM 馬達軸）；**GMR 磁性編碼器**抗碳刷粉塵/油污，適合有刷馬達環境；1:30 減速 → 輸出 100 RPM、輸出軸 60,000 counts/rev（位置解析度 0.006°），扭力足以做負載擾動實驗。
...
| 風險 | 影響 | 對策 |
|---|---|---|
| MG310 低速時 M 法計數少 | 低速速度量測量化誤差 | 500 PPR 已大幅改善；低速切 T 法（Encoder.c 設計） |
...
- **馬達模型**：一階慣性 + 摩擦死區 + 量測雜訊（對齊 MG310：輸出軸 100 RPM 上限，500 PPR GMR）
