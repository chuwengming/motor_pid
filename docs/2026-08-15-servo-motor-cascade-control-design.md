...
| 驅動器 | **DRV8871 直流馬達驅動模組**（TI）| NT$60 | **6.5–45V / 3.6A**（MG370 連續 <1A 綽綽有餘；堵轉靠**內建電流調節**保護，不需換參數）、**IPROPI 類比電流感測**（接 ADC → DOB/過流基礎）、**3.3V 邏輯相容**（F446 直連）、單晶片全 H-bridge、過流/過溫/短路保護 |
...
> **總計約 NT$1,090**（驅動器 DRV8871 較 BTN8982 便宜 ~NT$90）。
...
- **DRV8871（選定）**：TI 單晶片全 H-bridge（1 顆即完整），**3.6A 連續 + 內建電流調節**（MG370 堵轉自動限流保護——免費的 D 項保護）、**IPROPI 類比電流感測**（直接支持未來 DOB 擾動觀測器）、**3.3V 邏輯相容**（F446 直連免電平轉換）、價格最低（NT$60）。對本專案小馬達（<1A 連續）是「恰到好處」而非過度設計。
...
| 功能 | 針腳 | 外設/模式 | 說明 |
|---|---|---|---|
| 馬達 PWM | **PA8** | TIM1_CH1 | 20 kHz PWM（→ DRV8871 **IN1**） |
| 方向 | **PB0** | GPIO 輸出 | → DRV8871 **IN2**（HIGH=正轉 / LOW=反轉，配合 PMODE 接地） |
